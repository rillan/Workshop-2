...
public class MemberController implements Initializable   {
	...
	public void addMember(ActionEvent event)throws IOException{
		...
		List <Member> mPersonNumberChek = DB.members().findAll();
		for(Member mRCheck:mPersonNumberChek){
			if(mRCheck.personNumber.equals(newMember.personNumber)){
				System.out.println("Duplicate personalnumber");
				return;
			}
		}
		...
		if(cal.isLeapYear(yearToCalculate)){
			...
			if( personnumber.getText().matches(pn28day) || personnumber.getText().matches(pn30day) || personnumber.getText().matches(pn31day)){
				try{
					newMember = new Member(name.getText(),personnumber.getText());
					System.out.println(newMember.name + newMember.personNumber);
					DB.members().save(newMember);
	...
	private void cancel(ActionEvent event)throws IOException{
		...
		FXMLLoader loader = new FXMLLoader(Main.class.getResource("Application.fxml"));
		root = (Parent) loader.load();
		if(Main.getLoggedIn().matches(loggedIn)){
